...
// หลอดความอดทน: ตอนนี้ "มีผลจริง" แล้ว (Active Rush) — ปล่อยหมดขณะกำลังเสิร์ฟ = คอมโบขาด + เสียทิป ลูกค้าบึ้ง
// ตั้ง 11 วิให้ worker เดียวแบบ auto เสิร์ฟทันพอดี (เดินไป+คราฟต์+เดินกลับ ~7.8 วิ) แต่ช่วง Rush หด x0.6 = 6.6 วิ
// ทำให้ "ต้องกดผลิต" เร่งของถึงจะทันหลอด (getPatienceDurationMs() ใน features/rush.js คิดตัวคูณ Rush ให้)
const PATIENCE_DURATION_MS = 11000;
...
// เพิ่มจาก 150 -> 650: "กดผลิตสินค้า" เป็นแกนหลักของการเล่นแบบ Active Rush — แตะ 1 ครั้งดันคราฟต์
// ไปเยอะ(~1/4 ของคราฟต์ฐาน) ให้ผู้เล่นเร่งของทันออเดอร์ที่รุมเข้ามาได้ด้วยมือ (auto ยังทำงานช้าๆ เป็น fallback ตอน idle)
const STOKE_CRAFT_MS = 650;   // แตะ 1 ครั้ง = ดัน craftElapsed ของทุก worker ที่กำลังคราฟต์ไปข้างหน้าเท่านี้
const STOKE_SPARK_COUNT = 5;  // จำนวนประกายไฟที่กระเด็นออกจากจุดที่แตะ (ฟีดแบ็กภาพ)

// ===== Combo (เสิร์ฟต่อเนื่องให้ทันเวลา) — แกนความ "ต้องรักษาจังหวะ" ของ Active Rush =====
// เสิร์ฟออเดอร์สำเร็จภายใน COMBO_WINDOW จากครั้งก่อน = คอมโบ +1 (ตัวคูณเงิน + เติม Fever ไวขึ้น)
// ปล่อยให้ขาดช่วง หรือปล่อยหลอดความอดทนหมด = คอมโบขาด (เริ่มนับใหม่) — ทำให้ "เร็ว+ไม่พลาด" มีรางวัลจริง
const COMBO_WINDOW_MS = 3500;     // ต้องเสิร์ฟครั้งถัดไปภายในเวลานี้ ไม่งั้นคอมโบรีเซ็ต
const COMBO_GOLD_BONUS = 0.05;    // +5% เงินต่อ 1 คอมโบ (เหนือคอมโบแรก)
const COMBO_MAX_BONUS = 1.5;      // เพดานโบนัสเงินจากคอมโบ (+150% ที่คอมโบ ~31)
const COMBO_FEVER_BONUS = 0.04;   // เติมหลอด Fever ไวขึ้น +4% ต่อคอมโบ
const COMBO_MAX_FEVER_BONUS = 1.0;

// ===== Rush Hour (ลูกค้ารุมเป็นระลอก) — ช่วงกดดันที่ลูกค้ามาถี่ + หลอดหมดไว ต้องกดผลิตรัวๆ =====
const RUSH_INTERVAL_MIN_MS = 38000; // เว้นช่วงก่อน Rush ถัดไป (สุ่มในช่วง)
const RUSH_INTERVAL_MAX_MS = 60000;
const RUSH_DURATION_MS = 12000;     // Rush อยู่นานเท่านี้
const RUSH_SPAWN_MULT = 0.35;       // ระหว่าง Rush ลูกค้ามาถี่ขึ้น (spawn interval x นี้)
const RUSH_PATIENCE_MULT = 0.6;     // ระหว่าง Rush หลอดความอดทนหมดไวขึ้น (duration x นี้)
const RUSH_QUEUE_BONUS = 3;         // ระหว่าง Rush รับคิวได้มากขึ้น (ลูกค้ารุมเข้าเยอะ)
